...
import { Calendar, ArrowRight, Clock, Users, MapPin, Bell, Share2, Eye } from "lucide-react";
...
export function Programs() {
  ...
  const onSubmit = (data: RegistrationFormValues) => {
    console.log("Registration data:", data, "for program:", selectedProgram?.title);
  ...
  return (
    <section id="programs" className="py-20 bg-gradient-to-b from-white to-primary/5">
      <div className="container mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center justify-between mb-16">
          <div className="lg:w-1/2 text-center lg:text-left mb-8 lg:mb-0">
            ...
            <p className="text-gray-600 max-w-xl lg:pr-10">
              Enrich your spiritual journey and build meaningful connections. Our regular programs 
              are designed to nurture faith, education, and community bonds for all ages.
            </p>
          </div>
          <div className="lg:w-1/2 flex flex-col items-center lg:items-end gap-5">
            ...
            <div className="flex flex-wrap gap-3 justify-center lg:justify-end">
              <Badge 
                ...
                className={`px-4 py-2 rounded-full ${activeFilter === 'weekly' ? 'bg-primary text-white' : 'border-primary/30 text-primary'} font-medium hover:bg-primary/5 cursor-pointer transition-colors`}
                onClick={() => setActiveFilter(activeFilter === 'weekly' ? null : 'weekly')}
              >
                Weekly Classes
              </Badge>
              <Badge 
                variant={activeFilter === 'youth' ? 'default' : 'outline'} 
                className={`px-4 py-2 rounded-full ${activeFilter === 'youth' ? 'bg-primary text-white' : 'border-primary/30 text-primary'} font-medium hover:bg-primary/5 cursor-pointer transition-colors`}
                onClick={() => setActiveFilter(activeFilter === 'youth' ? null : 'youth')}
              >
                Youth Programs
              </Badge>
              <Badge 
                variant={activeFilter === 'family' ? 'default' : 'outline'} 
                className={`px-4 py-2 rounded-full ${activeFilter === 'family' ? 'bg-primary text-white' : 'border-primary/30 text-primary'} font-medium hover:bg-primary/5 cursor-pointer transition-colors`}
                onClick={() => setActiveFilter(activeFilter === 'family' ? null : 'family')}
              >
                Family Events
              </Badge>
              <Badge 
                variant={activeFilter === 'sisters' ? 'default' : 'outline'} 
                className={`px-4 py-2 rounded-full ${activeFilter === 'sisters' ? 'bg-primary text-white' : 'border-primary/30 text-primary'} font-medium hover:bg-primary/5 cursor-pointer transition-colors`}
                onClick={() => setActiveFilter(activeFilter === 'sisters' ? null : 'sisters')}
              >
                Sisters Activities
              </Badge>
            </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <div className="text-primary text-3xl mb-4">No matching programs found</div>
              ...
              >
                <Card className="overflow-hidden h-full flex flex-col border-transparent shadow-md hover:shadow-xl transition-shadow">
                  <div className="h-40 overflow-hidden relative">
                    <img 
                      src={program.image || "https://images.unsplash.com/photo-1584286596588-9d5f4523878c?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80"} 
                      alt={program.title} 
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/60 to-transparent flex flex-col justify-end p-5">
                      <h3 className="text-white text-xl font-serif font-bold">{program.title}</h3>
                    </div>
                  </div>
                  <CardContent className="p-5 flex-grow">
                    <div className="border-l-4 border-amber-400 pl-3 mb-4">
                      ...
                      <div className="flex items-center gap-2 text-primary font-semibold mb-1">
                        ...
                        <span className="text-sm ml-auto">{program.time || "After Isha"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-primary font-semibold mb-1">
                        <Users className="h-4 w-4" />
                        <span className="text-sm">LED BY:</span>
                        <span className="text-sm ml-auto">{program.instructor || "Imam Abdullah"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-primary font-semibold">
                        <MapPin className="h-4 w-4" />
                        <span className="text-sm">LOCATION:</span>
                        <span className="text-sm ml-auto">Main Prayer Hall</span>
                      </div>
                    </div>
                    <div className="mb-3">
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="sm:max-w-[500px]">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Please fill out this form to register for this program. We'll add you to our mailing list to keep you informed about this and future events.
                          </DialogDescription>
                        </DialogHeader>
                        <Form {...form}>
                    ...
                    <div className="flex space-x-1">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const promptEmail = () => {
                            const email = window.prompt("Please enter your email to receive notifications:");
                            if (email) {
                              if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                                toast({
        ...
        <div className="mt-16 text-center">
          ...
          <div className="mt-12 p-8 bg-primary/5 rounded-2xl max-w-3xl mx-auto">
            <h3 className="text-xl font-bold text-primary mb-4">Want to suggest a program?</h3>
            <p className="text-gray-600 mb-4">
              Have an idea for a community program or activity? We value your input and would love to hear your suggestions!
            </p>
            <Dialog>
              <DialogTrigger asChild>
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    We welcome your ideas! Fill out this form to suggest a new program for our community.
                  </DialogDescription>
                </DialogHeader>
                <form className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="idea-email" type="email" placeholder="your.email@example.com" />
                  </div>
                  <div className="space-y-2">
                    <FormLabel htmlFor="idea-title">Program Title</FormLabel>
                    <Input id="idea-title" placeholder="Proposed program title" />
                  </div>
                  <div className="space-y-2">
                    <FormLabel htmlFor="idea-description">Program Description</FormLabel>
                    <Textarea 
                      id="idea-description"
                  ...
                  <DialogFooter className="mt-6">
                    <Button 
                      ...
                      onClick={() => {
                        const nameInput = document.getElementById('idea-name') as HTMLInputElement;
                        const emailInput = document.getElementById('idea-email') as HTMLInputElement;
                        ...
                        toast({
                          title: "Thank You!",
                          description: "Your program idea has been submitted. We'll review it soon.",
                        });
